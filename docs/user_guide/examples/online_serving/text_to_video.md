...
For platform compatibility, RIFE interpolation runs on CPU by default on
non-CUDA/ROCm backends. On CUDA/ROCm it uses the current accelerator device when
available. To force a specific device for this post-processing step, set
`VLLM_OMNI_FRAME_INTERPOLATION_DEVICE`, for example `cpu`, `cuda`, `npu`, `xpu`,
or `musa`.
