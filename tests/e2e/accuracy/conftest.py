...
def pytest_addoption(parser):
    ...
    group.addoption(
        "--qwen-image-layered-image-source",
        action="store",
        default=None,
        help="Image source for Qwen-Image-Layered accuracy tests. Can be local path or remote URL.",
    )
...
@pytest.fixture(scope="session")
def qwen_image_layered_image_source(request: pytest.FixtureRequest) -> str | None:
    value = request.config.getoption("qwen_image_layered_image_source")
    return str(value) if value else None
