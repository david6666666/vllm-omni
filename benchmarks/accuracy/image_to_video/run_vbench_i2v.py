...
DEFAULT_NAME = "vbench_i2v"
DEFAULT_DIMENSIONS = list(VBENCH_I2V_DIMENSION_DISPLAY)
GENERATION_MANIFEST = "generation_manifest.json"
EVALUATION_MANIFEST = "evaluation_manifest.json"
SUMMARY_FILE = "summary.json"
SELECTED_FULL_INFO = "selected_vbench_i2v_full_info.json"
...
def _full_info_path(args: argparse.Namespace) -> Path:
    ...
    return args.vbench_root / "vbench2_beta_i2v" / "vbench2_i2v_full_info.json"
...
def _prepare_standard_entries(args: argparse.Namespace) -> list[dict[str, Any]]:
    if args.dataset_root is None:
        raise ValueError("Standard VBench-I2V mode requires --dataset-root.")
    full_info = load_json(_full_info_path(args))
    selected_entries = select_balanced_vbench_i2v_entries(
        full_info,
    ...
    for entry in selected_entries:
        prepared = dict(entry)
        prepared["custom_image_path"] = str(
            resolve_i2v_reference_path(args.dataset_root, str(entry["image_name"]), resolution=args.resolution)
        )
        prepared.pop("image_name", None)
        prepared_entries.append(prepared)
    return prepared_entries


def _prepare_custom_entries(args: argparse.Namespace) -> list[dict[str, Any]]:
    payload = load_json(args.custom_input_json)
    if not isinstance(payload, list):
        raise TypeError("Custom input JSON must be a list of case objects.")
    entries: list[dict[str, Any]] = []
    for item in payload:
        dimensions = item.get("dimension") or item.get("dimensions")
        if not isinstance(dimensions, list) or not dimensions:
            raise ValueError(f"Custom I2V case must provide non-empty dimensions: {item!r}")
        entries.append(
            {
                "prompt_en": str(item["prompt"]),
                "dimension": [str(dimension) for dimension in dimensions],
                "custom_image_path": str(Path(item["image_path"]).expanduser().resolve()),
            }
        )
    return entries
...
def _generate(args: argparse.Namespace) -> int:
    if args.custom_input_json is not None:
        selected_entries = _prepare_custom_entries(args)
        mode = "custom_input"
    else:
        selected_entries = _prepare_standard_entries(args)
    ...
    for entry in selected_entries:
        ...
        image_path = Path(entry["custom_image_path"])
        for sample_index in range(args.samples_per_case):
            ...
            form_fields, _ = build_i2v_video_request(
                prompt=prompt,
    ...
    manifest = {
        ...
        "dimensions": sorted({dimension for entry in selected_entries for dimension in entry["dimension"]}),
...
def _evaluate(args: argparse.Namespace) -> int:
    ...
    with vbench_import_context(args.vbench_root):
        ...
        bench = VBenchI2V(torch.device(args.device), str(selected_full_info_path), str(official_output_dir))
        bench.evaluate(
            videos_path=str(_videos_dir(args.output_root)),
            name=args.name,
...
def _summarize(args: argparse.Namespace) -> int:
    ...
    payload = {
        ...
        "evaluation": summarize_vbench_i2v_results(raw_results),
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run VBench-I2V evaluation against vLLM-Omni.")
    ...
    generate.add_argument("--dataset-root", type=Path, default=None)
    generate.add_argument("--custom-input-json", type=Path, default=None)
    generate.add_argument("--dimension", nargs="+", default=DEFAULT_DIMENSIONS)
    generate.add_argument("--cases-per-dimension", type=int, default=None)
    generate.add_argument("--samples-per-case", type=int, default=1)
    generate.add_argument("--resolution", type=str, default="1-1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "generate":
        if args.dataset_root is None and args.custom_input_json is None:
            parser.error("generate requires either --dataset-root or --custom-input-json")
        if args.dataset_root is not None and args.custom_input_json is not None:
            parser.error("generate accepts only one of --dataset-root or --custom-input-json")
