...
"""Verify a ModelOpt FP8 or NVFP4 diffusers checkpoint exported by vLLM-Omni
quantization helper scripts.

Three checks:
  A. transformer/config.json has a sane quantization_config block.
  B. transformer/*.safetensors contains expected quantized tensors.
...
def _check_config(transformer_dir: Path) -> tuple[int, str | None]:
    ...
    issues = []
    expected_method = SUPPORTED_ALGOS.get(quant_algo)
    if expected_method is None:
        issues.append(
            f"quant_algo={quant_algo!r} "
            f"(expected one of {tuple(SUPPORTED_ALGOS)} — vllm-omni adapter may not auto-detect)"
        )
    elif qc.get("quant_method") != expected_method:
        issues.append(f"quant_method={qc.get('quant_method')!r} (expected {expected_method!r})")
...
def _check_safetensors(transformer_dir: Path, quant_algo: str | None) -> int:
    ...
    for f in files:
        ...
        for k, info in header.items():
            ...
            if (
                (dtype.startswith("F8") or dtype == "U8")
                and k.endswith(".weight")
                and len(sample_quant_weight_keys) < 5
            ):
    ...
    for dtype, count in sorted(header_dtype_counts.items(), key=lambda kv: -kv[1]):
        marker = "  <-- quantized" if dtype.startswith("F8") or dtype == "U8" else ""
        print(f"    {dtype:10s} {count:>6d}{marker}")

    if quant_algo == "NVFP4":
        quant_count = header_dtype_counts.get("U8", 0)
        quant_label = "NVFP4-packed uint8"
    else:
        quant_count = sum(c for d, c in header_dtype_counts.items() if d.startswith("F8"))
        quant_label = "FP8"
    if quant_count == 0:
        print(f"[B] FAIL — no {quant_label} tensors on disk. Calibration likely did not quantize the weights.")
        return 1

    print(f"[B] PASS — {quant_count} {quant_label} tensors stored on disk.")
    ...
    print("    (Note: torch's get_tensor() may return quantized tensors differently across versions —")
    print("     irrelevant; vLLM's loader uses the checkpoint dtype and scale tensors.)")
...
def _check_size_vs_baseline(transformer_dir: Path, baseline: str | None) -> int:
    ...
    print(f"\n[C] Quantized transformer disk size: {quant_size:.2f} GiB")
    ...
    # Expected reduction: FP8 ~50%; NVFP4 can be smaller, depending on scale overhead.
    min_reduction = 30
    reduction = (1 - quant_size / bf16_size) * 100
    print(f"[C] BF16 baseline transformer disk size: {bf16_size:.2f} GiB ({bf16_dir})")
    print(f"[C] Disk reduction: {reduction:.1f}%  (quantized transformer is {quant_size / bf16_size:.0%} of BF16)")
    if reduction < min_reduction:
        print(f"[C] WARN — expected a material disk reduction; <{min_reduction}% suggests partial quantization.")
    return 0


def main() -> None:
    p = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument("--output", required=True, help="Path to the exported ModelOpt checkpoint root.")
    p.add_argument(
        "--baseline",
        default=None,
